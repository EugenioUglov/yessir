class ActionBlockController {
  ...
  bindClickBtnShowSettingsToCreateAdvancedActionBlock(handler) {
    this.view.bindClickBtnShowSettingsToCreateAdvancedActionBlock(
      handler
    );
  }

  bindClickBtnShowSettingsToCreateNote(handler) {
    this.view.bindClickBtnShowSettingsToCreateNote(
      handler
    );
  }

  bindClickBtnShowSettingsToCreateLink(handler) {
    this.view.bindClickBtnShowSettingsToCreateLink(
      handler
    );
  };
  ...
  #onClickBtnCreateActionBlockWithAutomation = (
    ...
    this.view.startLoading();
    this.#loadingHandler();
    this.createActionBlockWithOptimizedAutomationAsync(
      title,
      tagsPlusTitle,
      action,
      content,
  ...
  #bindViewEvenets() {
    this.view.bindClickBtnFixedPlus(
      this.switchStateMenuTypeActionBlocksToCreate
    );
    this.view.bindClickBtnCreateActionBlock(
      this.#onClickBtnCreateActionBlock
    );
    this.view.bindClickBtnCreateActionBlockWithAutomation(
      this.#onClickBtnCreateActionBlockWithAutomation
    );
    this.view.bindClickBtnOpenActionBlockSettings(
      this.openActionBlockSettings
    );
    this.view.bindClickBtnSaveEditedActionBlock(
      this.updateActionBlock
    );
    this.view.bindClickBtnSaveQuicklyEditedActionBlock(
      this.updateQuicklyEditedActionBlock
    );
    this.view.bindClickBtnDeleteActionBlock(
      this.deleteActionBlock
    );
    this.view.bindClickBtnRewriteActionBlocks(
      this.rewriteActionBlocks
    );
    this.view.bindClickBtnCreateDefaultActionBlocks(
      this.updateDefaultActionBlocks
    );

    this.view.bindClickBtnShowSettingsToCreateFolder(
      this.showSettingsToCreateFolder
    );

    this.view.bindClickBtnCancelSettings(
    ...
    const getSingularizedWordsPromise = new Promise((resolve, reject) => {
      ...
      nounNumber.getSingularizedWords(
        ...
        (singularizedWords) => {
          resolve({
            status: "success",
            singularized_words: singularizedWords,
          });
        },
        (error) => {
  ...
  showActionBlocks(
    actionBlocksToShow,
    countActionBlocksToShowAtTime = 50
  ) {
    const that = this;
    ...
    for (const [
      ...
      if (i >= countActionBlocksToShowAtTime - 1) {
        break;
      }
    ...
    that.#indexLastShowedActionBlock = i;
  ...
  getIndexLastShowedActionBlock() {
    return this.#indexLastShowedActionBlock;
  }
  ...
  getScrollPositionOnExecuteBlock() {
    return this.#scrollPositionOnExecuteBlock;
  }
  ...
  addOnPageNextActionBlocks() {
    ...
    let countActionBlocksCurr = 0;
    let maxCountActionBlocksToAddOnPage = 50;
    ...
    for (
      i = this.#indexLastShowedActionBlock;
      i < actionBlocks.length;
      i++
    ) {
      if (countActionBlocksCurr >= maxCountActionBlocksToAddOnPage) {
        break;
      }

      this.showActionBlock(actionBlocks[i]);
      countActionBlocksCurr++;
    }

    this.#indexLastShowedActionBlock = i;
